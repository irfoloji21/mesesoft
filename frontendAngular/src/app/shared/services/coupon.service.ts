...
export class CouponService {
  ...
  createCouponCode(couponData: any): Observable<CouponCode> {
    return this.http.post<CouponCode>(
      `${this.baseUrl}/coupon/create-coupon-code`,
      couponData
    );
  }

  getCouponsByShopId(shopId: string): Observable<CouponCode> {
    return this.http.get<CouponCode>(
      `${this.baseUrl}/coupon/get-coupon/${shopId}`
    );
  }

  deleteCoupon(couponId: string): Observable<CouponCode> {
    return this.http.delete<CouponCode>(
      `${this.baseUrl}/coupon/delete-coupon/${couponId}`
    );
  }
  getCouponById(couponId: string): Observable<CouponCode> {
    return this.http.get<CouponCode>(
      `${this.baseUrl}/coupon/get-coupon-code/${couponId}`
    );
  }

  getCouponValueByName(couponName: any): Observable<any> {
    return this.http.get<CouponCode>(
      `${this.baseUrl}/coupon/get-coupon-value/${couponName}`
    );
  }

...
export class CollectionSidebarComponent implements OnInit {
  ...
  constructor(
    ...
    this.route.queryParams.subscribe((params) => {
      this.filteredIds = params.filteredIds
        ...
        .subscribe((response: any) => {
            ...
            this.categoryService.getCategories().subscribe((data: any) => {
              if (data.success) {
                ...
                this.products = categoriesArray.filter((item: any) =>
                  parentCategoryIds.includes(item._id)
                );
              }
            });
          }
          ...
          if (this.theMostLiked.length > 0) {
            ...
            this.products = this.products.filter((item) =>
              this.theMostLiked.includes(item._id)
            );
          }
          ...
          if (this.filteredIds && this.filteredIds.length > 0) {
            this.products = this.products.filter((item) =>
              this.filteredIds.includes(item._id)
            );
          }
          ...
          if (this.Megamenu) {
            this.products = this.products.filter((item) =>
              this.Megamenu.includes(item._id)
            );
          }
          ...
          this.products = this.products.filter(
            (item) =>
              item.discountPrice >= this.minPrice &&
              item.originalPrice <= this.maxPrice
          );
